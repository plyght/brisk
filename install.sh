...
REPO="plyght/brisk"
INSTALL_DIR="${BRISK_INSTALL_DIR:-$HOME/.local/bin}"
...
path_hint() {
  if command -v brisk &>/dev/null 2>&1; then
    return
  fi
  printf "\n${BOLD}Add brisk to your PATH:${NC}\n"
  case "${SHELL:-}" in
...
install_from_repo() {
  ...
  if ! command -v cargo &>/dev/null; then
    die "cargo not in PATH — install Rust from https://rustup.rs/ or use BRISK_USE_RELEASE=1 to download a binary."
  fi
  info "Building brisk from local checkout (${root})…"
  ( cd "$root" && cargo build --release )
  mkdir -p "$INSTALL_DIR"
  cp "${root}/target/release/brisk" "${INSTALL_DIR}/brisk"
  chmod +x "${INSTALL_DIR}/brisk"
  ok "brisk built and installed to ${INSTALL_DIR}/brisk"
  path_hint
}
...
install_from_release() {
  ...
  info "Installing brisk ${VERSION} (${os}/${arch}) from GitHub Releases…"
  ...
  mkdir -p "$INSTALL_DIR"
  mv "$TMP" "${INSTALL_DIR}/brisk"

  ok "brisk ${VERSION} installed to ${INSTALL_DIR}/brisk"
  path_hint
}

_src="${BASH_SOURCE[0]:-}"
if [[ -n "$_src" ]] && [[ "$(basename -- "$_src")" == "install.sh" ]] && [[ "${BRISK_USE_RELEASE:-}" != "1" ]]; then
  _root="$(cd "$(dirname -- "$_src")" && pwd)"
  if [ -f "${_root}/Cargo.toml" ] && grep -q 'name = "brisk"' "${_root}/Cargo.toml"; then
